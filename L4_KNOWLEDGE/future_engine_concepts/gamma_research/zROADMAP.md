...
Real collection from one real options venue for one real
underlying. Canonical facts only — no GEX computation yet.

### Phase 2 — GEX Computation
Derived, disposable gamma exposure calculation from Phase 1's
canonical data.
...
source that the abstraction is sound — not before.
