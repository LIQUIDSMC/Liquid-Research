...
CANONICAL_OUTPUT_PATH = os.path.join(REPO_ROOT, "data", "approved_markets", "prediction_markets_latest.csv")
...
STAGES = [
    ("market_collector", lambda py: [py, "collectors/market_collector.py"], TIMEOUT_MARKET_COLLECTOR),
    ...
    ("scanner", lambda py: [py, "scanner/scanner.py"], TIMEOUT_SCANNER),
    # PHASE 2 TODO: after this stage succeeds, verify a fresh
    ...
    ("paper_trader", lambda py: [py, "simulator/paper_trader.py"], TIMEOUT_PAPER_TRADER),
    ...
    ("paper_resolver", lambda py: [py, "simulator/paper_resolver.py"], TIMEOUT_PAPER_RESOLVER),
    # PHASE 2 TODO: after this stage succeeds, verify the paper-trade
    # file remains readable, and that status counts remain internally
    # consistent (open + closed == total, wins + losses == closed).
    # Not implemented in Phase 1.
    ("run_obi", lambda py: [py, "programs/program_b/diagnostics/run_obi.py"], TIMEOUT_RUN_OBI),
    ...
    # Not implemented in Phase 1.
    ("run_near_book_depth", lambda py: [py, "programs/program_b/diagnostics/run_near_book_depth.py"], TIMEOUT_RUN_NEAR_BOOK_DEPTH),
